{
  ...
  {- if and (.Values.metrics.prometheus) (not (.Values.metrics.prometheus.serviceMonitor).enabled)
        ...
        prometheus.io/port: {
  { quote (index .Values.ports .Values.metrics.prometheus.entryPoint).port
  }
}
      {
  {- end
  ...
  {- end
      ...
      serviceAccountName: {
  { include "traefik.serviceAccountName" .
  }
}
  ...
  { $name | quote
          ...
          containerPort: {
  { default $config.port $config.containerPort
  }
}
          {
  {- if $config.hostPort
  ...
  {- end
          ...
          protocol: {
  { default "TCP" $config.protocol | quote
  }
}
          {
  ...
  {- if $config.hostPort
          ...
          hostPort: {
  { default $config.hostPort $config.http3.advertisedPort
  }
}
           {
  {- end
  ...
  {- $listenAddr := default ":9943" .Values.hub.apimanagement.admission.listenAddr
        ...
        - name: admission
          containerPort: {
  { last (mustRegexSplit ":" $listenAddr 2)
  }
}
          protocol: TCP
          {
...
" $domain.sans }}"{
  ...
  {- if $config.forwardedHeaders.trustedIPs
          ...
          - "--entryPoints.{{ $entrypoint }}.forwardedHeaders.trustedIPs={{ join ",
" $config.forwardedHeaders.trustedIPs }}"{
  {- end
  }
}
              {
  ...
  {- if $config.proxyProtocol.trustedIPs
          ...
          - "--entryPoints.{{ $entrypoint }}.proxyProtocol.trustedIPs={{ join ",
" $config.proxyProtocol.trustedIPs }}"{
  {- end
  }
}
              {
  ...
  {- range $field, $value := $setting
          ...
          - "--certificatesresolvers.{{ $resolver }}.acme.{{ $option }}.{{ $field }}={{ if kindIs "slice" $value }}{{ join ",
" $value }}{{ else }}{{ $value }}{{ end }}"{
  ...
  {- if .Values.persistence.enabled
          ...
          persistentVolumeClaim:
            claimName: {
  { default (include "traefik.fullname" .) .Values.persistence.existingClaim
  }
}
          {
  {- else
